...
unique(fake_news$title_has_excel)
```
...
-   The prior probability (an unconditional probability) that the article is real is 0.60
...
```{r}
prior_probability_article_exc <- 0.12
```

P(Ac) = 0.88

-   The prior probability (an unconditional probability) that the article does not have an exclamation point is 0.8
...
```{r}
(prior_probability_article_fake * likelihood_article_fake_has_exc) /
  prior_probability_article_exc
```
...
-   The prior probability (an [unconditional probability](https://www.bayesrulesbook.com/chapter-2#building-a-bayesian-model-for-events:~:text=two%20events.%20The-,unconditional%20probability,-of)) that the people say pop in general
...
```{r}
prior_probability_region <- 0.24
```

The people that are from the south
...
People from the south that says pop

```{r}
likelihood_probability_pop_region <- 0.079
```

P(S\|A) = P(A)L(A\|S)/P(A)

```{r}
(prior_probability_pop_true * likelihood_probability_pop_region) /
  prior_probability_region
```
